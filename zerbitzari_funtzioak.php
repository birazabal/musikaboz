...
class erabiltzaileKontrola{
	function login_kontrola(){
		try{
			if ((isset($_POST["izena"])) && (isset($_POST["pass"]))){
   				if ($_POST["izena"] != "" && $_POST["pass"] != ""){	
					...
					$db = new Datubasea();
					//funtzioa erabili
					$emaitza = $db->begiratu_erab($izena,$pass);
					if ($emaitza == "wtf"){
						$mezua = "ERABILTZAILE IZENA edo PASAHITZA gaizki daude" . $emaitza;		
					}else{
						...
						$mezua = "ondo";
						//header("Location:http://192.168.0.157/musikaboz/musikaboz bezero/");
